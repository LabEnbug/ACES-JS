...
function Promote() {
  ...
  const perPrice = 0.3; // Promotion per visit price
  ...
  function GetVideoInfo() {
    baxios
      ...
      .then((response) => {
        ...
        setVideoInfo(video);
        setRemainPromoteCount(data.data.remain_advertise_count)
              ...
              {current === 1 && (
                <Form.Item noStyle>
                  ...
                  <Form.Item
                    ...
                    initialValue="1"
                    field="count"
                    extra={'要推送的广告的数量，当用户播放一次该视频，则扣除一次流量数'}
                    rules={[
                      {
                        required: true,
                        message: '请填写流量数',
              ...
              {current === 2 && (
                <Form.Item noStyle>
                  <Result
                    status="success"
                    title={'广告投放成功'}
                    subTitle={'成功为视频充值广告投放流量 ' + form.getFieldValue("count") + ' 次'}
                    extra={[
                      <Button
                        key="watchPage"
